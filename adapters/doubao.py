...
class DoubaoAdapter(BaseAdapter):
    ...
    async def _call_api(self, messages: List[Dict[str, str]]) -> Optional[str]:
        ...
        async with session.post(
            ...
            json={"model": "Doubao-3", "messages": messages, "stream": False},
    ...
    async def _call_api_stream(self, messages: List[Dict[str, str]]) -> AsyncGenerator[str, None]:
        ...
        async with session.post(
            self.model_info["api_url"],
            json={"model": "Doubao-3", "messages": messages, "stream": True},
            headers=self._get_api_headers()
        ) as resp:
            if resp.status == 200:
    ...
    async def _mock_response(self, messages: List[Dict[str, str]]) -> str:
        ...
        return f"[模拟响应] 你好！我是豆包。你问的是：「{user_msg}」\n\n豆包是字节跳动开发的智能助手，可以回答各种问题。当前为模拟响应模式，配置 API Key 后可获得真实回复。"
